...
* Credits
Initially this configuration was taken from [[https://github.com/freetonik/emacs-dotfiles][freetonik/emacs-dotfiles]] repo
* Installing
...
Dracula seems to be perfect theme for everything.
...
Liga Hack font, remove the cruft and make the initial size bigger.
...
Kill line with =s-Backspace=, which is =Cmd+Backspace= by default.
Note that thanks to Simpleclip, killing doesn't rewrite the system clipboard.
Kill one word by =M+Backspace=.
...
This plugin is made to generate html from org-mode files
...
Store all my org files in =~/Dropbox/me.txt/org=.
